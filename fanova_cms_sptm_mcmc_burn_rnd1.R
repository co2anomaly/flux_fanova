...
# Temporary
#config = "fanova_cms_eurasia_jja_sptm.csv"
#chain = 2
...
## Temporary - Read directly from RData likelihood output
...
for (g1 in seq(1,length(gpgrps))) {
    ...
    if (gpgrps[g1] == "noise") {
        pr_gp_nu[[gpgrps[g1] ]] = c(prinfo$prior[[ gpgrps[g1]]][["ar_correlation"]][["shape1"]],
...
#          jpsd_mh[[gpgrps[g1] ]] = rand_mh_rate(acrt_mh[[gpgrps[g1] ]], jpsd_mh[[gpgrps[g1] ]])
  ...
  if (nmnA > 2) {
    a.cur = samp_post_vecchia_mltlev(y.tilde.a,cvprlst$mainA,W.eps,vec.spec,nrp,nmnA*nmnB,nloc)
  }
  else {
  ...
  if (nmnB > 2) { 
    b.cur = samp_post_vecchia_mltlev(ytilde.b,cvprlst$mainB,W.eps,vec.spec,nrp,nmnA*nmnB,nloc)
  }
  else {
