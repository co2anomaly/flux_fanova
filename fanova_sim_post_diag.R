# Posterior diagnostics for functional ANOVA
...
gpbnms = paste0("Examples/sp_pars_",gpgrps,"_post.png")
gprfrm = data.frame(vrnm=gpgrps,ofile=gpbnms,nrow=rep(2,5),width=rep(1200,5),height=rep(900,5),stringsAsFactors = FALSE)
d_ply(gprfrm,c("vrnm"),.fun=gp_partrace_post,.progress="text",cflst=cfglst,pltthm=theme_mat)

# MH Acceptance
mhfrm = gprfrm
mhfrm$width=rep(750,5)
mhfrm$height=rep(600,5)
mhfrm$ofile = paste0("Examples/mh_sp_pars_",gpgrps,"_post.png")
...
fldfrm$ofile = paste0("Examples/field_locs_",gpgrps[1:4],"_post.png")
d_ply(fldfrm,c("vrnm"),.fun=gp_fldtrace_post,.progress="text",cflst=cfglst,pltthm=theme_mat)
