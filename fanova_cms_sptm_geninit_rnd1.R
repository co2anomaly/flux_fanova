...
lcidx = seq(1,nloc)
cftst = fit_anova_mltlev_arr3d(1,dtarr,contrstA=ctrstA,contrstB=ctrstB)
cfblst = map(.x=lcidx,.f=fit_anova_mltlev_arr3d, datarr=dtarr, contrstA=ctrstA, contrstB=ctrstB)
cfbsln = matrix(unlist(cfblst), ncol=nmnA*nmnB, byrow=TRUE)
...
for (g1 in seq(1,length(gpgrps))) {
  ...
  if (gpgrps[g1] == "noise") {
    ...
    rholn = paste0("Temporal autocorrelation for ",gpgrps[g1])
    varlst[[vct]] = ncvar_def(rhonm,"None",list(dimscl),-9999,
                              longname=rholn)
  }
}
...
for (j in seq(1,nchain)) {
    ...
    for (g1 in seq(1,length(gpgrps))) {
        ...
        if (gpgrps[g1] == "noise") {
            rhonm = paste0("ar_correlation_",gpgrps[g1])
            prprs = prinfo$prior[[ gpgrps[g1] ]][[ "ar_correlation" ]]
            if (prprs$form == "beta") {
                psmp = prprs$lim1 + (prprs$lim2 - prprs$lim1) * rbeta(1, shape1 = prprs$shape1, shape2 = prprs$shape2)
            }
            ncvar_put(nc1,rhonm,psmp)
        }
    }
