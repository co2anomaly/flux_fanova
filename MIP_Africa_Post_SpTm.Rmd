...
## MIP Africa Example Functional ANOVA

### 

Burn-in diagnostics for functional ANOVA MCMC run, OCO-2 V9 MIP over Africa  
...
exmpcfg = read.csv("config/fanova_mip_africa_prdev_sptm.csv",header = TRUE,stringsAsFactors = FALSE)
...
for (i in seq(1,nrow(gprfrm))) {
    gp_partrace_post(gprfrm[i,],cflst=cfglst,pltthm=theme_mat,nstrt=1001)
}
```
...
mhfrm$ofile = paste0(cfglst$output_dir,"/mip_mh_sp_pars_",gpgrps,"_post.png")
for (i in seq(1,nrow(gprfrm))) {
    aratebox(mhfrm[i,],cflst=cfglst,pltthm=theme_mat)
}
```
...
for (i in seq(1,nrow(fldfrm))) {
    gp_fldtrace_post(fldfrm[i,],cflst=cfglst,pltthm=theme_mat,nstrt=101)
}
```
...
numn = 0.25
numx = 3.75
gcvsmn = gp_parcov_post(vrnm="mean",cflst=cfglst,nstrt=1001)
...
```{r gp_cov_mainA}
gcvsa = gp_parcov_post(vrnm="mainA",cflst=cfglst,nstrt=1001)
# Adjust for logit
gcvsa[,3,] = exp(gcvsa[,3,])
...
```{r gp_cov_mainB}
gcvsb = gp_parcov_post(vrnm="mainB",cflst=cfglst,nstrt=1001)
# Adjust for logit
gcvsb[,3,] = exp(gcvsb[,3,])
...
```{r gp_cov_interact}
gcvsi = gp_parcov_post(vrnm="interact",cflst=cfglst,nstrt=1001)
# Adjust for logit
gcvsi[,3,] = exp(gcvsi[,3,])
...
gcvsns = gp_parcov_post(vrnm="noise",cflst=cfglst,nstrt=1001,logpar=FALSE,arparam=TRUE)
# Adjust for logit
...
muarr = gp_parcov_post(vrnm="mean",cflst=cfglst,nstrt=chnst,logpar=FALSE)
dimnames(muarr) = list(Iteration = seq(chnst,nsamp), Var = c("VarStdDev","VarRange","VarSmooth"),
                            Chain = sprintf("Chn%d",seq(1,nchain)) )
pfrm = as_tibble(muarr, rownames="Iteration") 
...
smrymn = pfrm %>% group_by(Variable) %>% summarise(pstsmry_tidy(Sample))
print(smrymn)
```
...
```{r gp_smry_mainA}
aarr = gp_parcov_post(vrnm="mainA",cflst=cfglst,nstrt=chnst,logpar=FALSE)
dimnames(aarr) = list(Iteration = seq(chnst,nsamp), Var = c("VarStdDev","VarRange","VarSmooth"),
                            Chain = sprintf("Chn%d",seq(1,nchain)) )
pfrm = as_tibble(aarr, rownames="Iteration") 
pfrm = pfrm %>% pivot_longer(cols = starts_with("Var"), names_to = "VarChn",
                             names_prefix="Var", values_to = "Sample")
pfrm = pfrm %>% separate(VarChn, c("Variable","Chain"), sep = "\\.", remove=FALSE)
pfrm = pfrm %>% mutate_at(c('Iteration'), as.numeric)
smrya = pfrm %>% group_by(Variable) %>% summarise(pstsmry_tidy(Sample))
print(smrya)
```
...
```{r gp_smry_mainB}
barr = gp_parcov_post(vrnm="mainB",cflst=cfglst,nstrt=chnst,logpar=FALSE)
dimnames(barr) = list(Iteration = seq(chnst,nsamp), Var = c("VarStdDev","VarRange","VarSmooth"),
                            Chain = sprintf("Chn%d",seq(1,nchain)) )
pfrm = as_tibble(barr, rownames="Iteration") 
pfrm = pfrm %>% pivot_longer(cols = starts_with("Var"), names_to = "VarChn",
                             names_prefix="Var", values_to = "Sample")
pfrm = pfrm %>% separate(VarChn, c("Variable","Chain"), sep = "\\.", remove=FALSE)
pfrm = pfrm %>% mutate_at(c('Iteration'), as.numeric)
smryb = pfrm %>% group_by(Variable) %>% summarise(pstsmry_tidy(Sample))
print(smryb)
```
...
```{r gp_smry_interact}
abarr = gp_parcov_post(vrnm="interact",cflst=cfglst,nstrt=chnst,logpar=FALSE)
dimnames(abarr) = list(Iteration = seq(chnst,nsamp), Var = c("VarStdDev","VarRange","VarSmooth"),
                            Chain = sprintf("Chn%d",seq(1,nchain)) )
pfrm = as_tibble(abarr, rownames="Iteration") 
pfrm = pfrm %>% pivot_longer(cols = starts_with("Var"), names_to = "VarChn",
                             names_prefix="Var", values_to = "Sample")
pfrm = pfrm %>% separate(VarChn, c("Variable","Chain"), sep = "\\.", remove=FALSE)
pfrm = pfrm %>% mutate_at(c('Iteration'), as.numeric)
smryab = pfrm %>% group_by(Variable) %>% summarise(pstsmry_tidy(Sample))
print(smryab)
```
...
```{r gp_smry_noise}
epsarr = gp_parcov_post(vrnm="noise",cflst=cfglst,nstrt=chnst,logpar=FALSE,arparam=TRUE)
dimnames(epsarr) = list(Iteration = seq(chnst,nsamp), Var = c("VarStdDev","VarRange","VarSmooth","VarARCorr"),
                            Chain = sprintf("Chn%d",seq(1,nchain)) )
pfrm = as_tibble(epsarr, rownames="Iteration") 
pfrm = pfrm %>% pivot_longer(cols = starts_with("Var"), names_to = "VarChn",
                             names_prefix="Var", values_to = "Sample")
pfrm = pfrm %>% separate(VarChn, c("Variable","Chain"), sep = "\\.", remove=FALSE)
pfrm = pfrm %>% mutate_at(c('Iteration'), as.numeric)
smryeps = pfrm %>% group_by(Variable) %>% summarise(pstsmry_tidy(Sample))
print(smryeps)
```
