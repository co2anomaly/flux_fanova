---
title: "MIP Africa Stationary MCMC"
author: "ANOVA User"
date: "Aug 30, 2022"
...
Stationary posterior diagnostics for functional ANOVA MCMC run  
OCO-2 V9 Flux MIP, Africa region  
...
exmpcfg = read.csv("config/fanova_mip_africa_prdev.csv",header = TRUE,stringsAsFactors = FALSE)
...
d_ply(gprfrm,c("vrnm"),.fun=gp_partrace_post,cflst=cfglst,pltthm=theme_mat,nstrt=1001)
```
...
```{r gp_cov_noise}
gcvsns = gp_parcov_post(vrnm="noise",cflst=cfglst,nstrt=1001)
# Adjust for logit
...
muarr = gp_parcov_post(vrnm="mean",cflst=cfglst,nstrt=1001,logpar=FALSE)
...
```{r gp_smry_mainA}
aarr = gp_parcov_post(vrnm="mainA",cflst=cfglst,nstrt=1001,logpar=FALSE)
...
```{r gp_smry_mainB}
barr = gp_parcov_post(vrnm="mainB",cflst=cfglst,nstrt=1001,logpar=FALSE)
...
```{r gp_smry_interact}
abarr = gp_parcov_post(vrnm="interact",cflst=cfglst,nstrt=1001,logpar=FALSE)
...
```{r gp_smry_noise}
epsarr = gp_parcov_post(vrnm="noise",cflst=cfglst,nstrt=1001,logpar=FALSE)
