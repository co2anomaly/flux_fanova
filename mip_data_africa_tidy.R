...
miprgns = rgn_tbl %>% filter(Region %in% c(6,7,8,9))
...
prfnl = fklon_tbl(prfnl,lonvar = "Longitude",center=15)
...
pnm = paste0(cfglst$output_dir,"/MIP_Africa_2016_JJA_Prior.pdf")
...
mipfnl = fklon_tbl(mipfnl,lonvar = "Longitude",center=15)
tmlbls = c("6_Jun","7_Jul","8_Aug")
...
for (i in seq(1,length(expt))) {
  ...
  gmn = ggplot(frmsb,aes(x=fk360,y=Latitude)) + geom_tile(aes(fill=FluxPlt)) + 
    ...
    scale_y_continuous("",breaks=ltlb$origlat,labels=parse(text=ltlb$labxpr),limits=c(-40,40)) + 
    ggtitle(ttlstr) + theme_mat + coord_equal()
  pnm = paste0(cfglst$output_dir,"/MIP_Africa_2016_JJA_",expt[i],".pdf")
  pdf(pnm,width = 12,height=7)
  print(gmn)
  dev.off()
}
...
for (i in seq(1,length(expt))) {
  ...
  gmn = ggplot(frmsb,aes(x=fk360,y=Latitude)) + geom_tile(aes(fill=FluxPlt)) + 
    ...
    scale_y_continuous("",breaks=ltlb$origlat,labels=parse(text=ltlb$labxpr),limits=c(-40,40)) + 
    ggtitle(ttlstr) + theme_mat + coord_equal()
  pnm = paste0(cfglst$output_dir,"/MIP_Africa_2016_JJA_PrDev_",expt[i],".pdf")
  pdf(pnm,width = 12,height=7 )
  print(gmn)
  dev.off()
}
