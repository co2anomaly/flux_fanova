...
# Subset
miprgns = miprgns[(miprgns$Region == 1) | (miprgns$Region == 2),]
...
mipfnl = fklon(mipfnl,lonvar = "Longitude",center=-90)
...
for (i in seq(1,length(expt))) {
    ...
    pnm = paste0(cfglst$output_dir,"/NorthAmerica_2016_JJA_",expt[i],".pdf")
...
prfnl = fklon(prfnl,lonvar = "Longitude",center=-90)
tmlbls = c("6_Jun","7_Jul","8_Aug")
...
gmn = ggplot(frmsb,aes(x=fk360,y=Latitude)) + geom_tile(aes(fill=FluxPlt)) + 
    ...
    scale_y_continuous("",breaks=ltlb$origlat,labels=parse(text=ltlb$labxpr),limits=c(10,84)) + 
    ggtitle(ttlstr) + theme_mat + coord_equal()
pnm = paste0(cfglst$output_dir,"/NorthAmerica_2016_JJA_Prior.pdf")
pdf(pnm,width = 12,height=7)
print(gmn)
dev.off()
...
for (i in seq(1,length(expt))) {
  ...
  gmn = ggplot(frmsb,aes(x=fk360,y=Latitude)) + geom_tile(aes(fill=FluxPlt)) + 
    ...
    scale_y_continuous("",breaks=ltlb$origlat,labels=parse(text=ltlb$labxpr),limits=c(10,84)) + 
    ggtitle(ttlstr) + theme_mat + coord_equal()
  pnm = paste0(cfglst$output_dir,"/NorthAmerica_2016_JJA_PrDev_",expt[i],".pdf")
...
pnm = paste0(cfglst$output_dir,"/MIP_ANOVA_Summary_Mean_JJA_2016.pdf")
pdf(pnm,width = 12,height=8)
print(gmn)
dev.off()
