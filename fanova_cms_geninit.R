...
exmpcfg = read.csv("fanova_cms_pacific.csv",header = TRUE,stringsAsFactors = FALSE)
...
dimeffB = ncdim_def("effect_B","",seq(1,nmnA-1),create_dimvar = FALSE)
...
cfbsln = aaply(dtarr,1,.fun = fit_anova_arr3d, .progress="text", contrstA=ctrstA, contrstB=ctrstB)
            ...
            if (prprs$form == "halfnormal") {
                ...
                if (gpvnms[k] == "gp_stddev") {
                    if (gpgrps[g1] == "noise") {
                        ...
                        psmp = exp(log(psmp) * 0.4 + 0.6 * log(cfsd)) 
                    }
                }
            }
