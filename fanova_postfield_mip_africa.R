...
mumrg = fklon(mumrg,lonvar = "Longitude",center=15)
...
pnm = paste0(cfglst$output_dir,"/MIP_ANOVA_Posterior_Africa_PrDev_Mu.pdf")
...
pnm = paste0(cfglst$output_dir,"/MIP_ANOVA_Posterior_Africa_PrDev_Mu_StdDev.pdf")
pdf(pnm,width = 12,height=8)
print(gstd)
dev.off()
...
bmrg = fklon(bmrg,lonvar = "Longitude",center=15)
...
pnm = paste0(cfglst$output_dir,"/MIP_ANOVA_Posterior_Africa_PrDev_BEff.pdf")
...
muqmrg = fklon(muqmrg,lonvar = "Longitude",center=15)
...
pnm = paste0(cfglst$output_dir,"/MIP_ANOVA_Posterior_Africa_PrDev_Mu_Cred.pdf")
pdf(pnm,width = 10,height=5)
print(gmuq)
dev.off()
...
ctrqmrg = fklon(ctrqmrg,lonvar = "Longitude",center=15)
gctrq = ggplot(ctrqmrg,aes(x=fk360,y=Latitude)) + geom_tile(aes(fill=value)) + 
  geom_path(aes(x=fk360,y=Y,group=group2), data=wrld, size=0.8, color="#444444") +
  scale_fill_gradientn("Flux",colors=r7,limits=c(-30,30))  + 
  facet_wrap( ~ Pctile,nrow=1) + 
  scale_x_continuous("",breaks=lnlb$fk360,labels=parse(text=lnlb$labxpr),limits=c(-35,40)) + 
  scale_y_continuous("",breaks=ltlb$origlat,labels=parse(text=ltlb$labxpr),limits=c(-40,40)) + 
  ggtitle("MIP Flux ANOVA Model Contrast Interval") + theme_mat + coord_equal()
pnm = paste0(cfglst$output_dir,"/MIP_ANOVA_Posterior_Africa_PrDev_AEff_Contrast.pdf")
pdf(pnm,width = 10,height=5)
print(gctrq)
dev.off()
...
pnm = paste0(cfglst$output_dir,"/MIP_ANOVA_Posterior_Africa_PrDev_BEff_Contrast.pdf")
pdf(pnm,width = 10,height=5)
print(gctrqb)
dev.off()
